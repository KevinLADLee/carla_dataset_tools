...
def main():
    ...
    try:
        ...
        while True:
            ...
            world.tick()
            w_frame = world.get_snapshot().timestamp.elapsed_seconds
            print("\nWorld's frame: %f" % w_frame)
            ...
            try:
                for _ in range(len(sensor_list)):
                    ...
                    if s_frame[1].startswith('lidar'):
                        s_frame[2].save_to_disk("_out/{}_{}.ply".format(s_frame[0], s_frame[1]))
                    else:
                        s_frame[2].save_to_disk("_out/{}_{}.png".format(s_frame[0], s_frame[1]))
                    print("    Frame: %d Sensor: %s      T: %f " % (s_frame[0], s_frame[1], s_frame[2].timestamp))
                print("loop duration: {}".format(time.time()-loop_start))
            except Empty:
